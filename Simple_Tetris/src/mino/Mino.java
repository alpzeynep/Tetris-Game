...
public class Mino {
	...
	public void update() {
		...
		if(KeyHandler.downPressed) {
			if(bottomCollision == false) {
				...
				// When the moved down reset the autoDropCounter
				autoDropCounter = 0;
			}
		...
		if(bottomCollision) {
			...
			if(autoDropCounter == PlayManager.dropInvertal) {
				//the mino goes down
				b[0].y += Block.SIZE;
				b[1].y += Block.SIZE;
				b[2].y += Block.SIZE;
	...
	private void deactivating() {
		...
		if(deactiveCounter == 45) {
			...
			// if the bottom still hitting after 45 frames, deactivate the mino 
			if(bottomCollision) {
				activate = false;
			}
		}
